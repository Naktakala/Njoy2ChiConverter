...
def BuildCombinedData(raw_njoy_data, plot=False):
  ...
  # Adding all the elastic freegas scattering data
  ...
  def AddTransferNeutron(data_vals,offset=0,additive=True,verbose=False):
    for entry in data_vals:
      ...
      for m in range(0,num_moms):
        ...
        if verbose and m==0:
          print("gprime g v {:d} {:d} {:f}".format(gprime,g,v))
  ...
  transfer_sig_s_el = np.copy(transfer_mats[0])
  ...
  transfer_sig_s_el_freegas = np.copy(transfer_mats[0])
  ...
  for range_data in nranges_to_nranges_elastic_sab:
    AddTransferNeutron(range_data,additive=False,verbose=True)
  sig_s_el_sab = transfer_mats[0] @ np.ones(G)

  transfer_sig_s_el_sab = np.copy(transfer_mats[0])

  for g in range(0,G):
    transfer_sig_s_el_freegas[g,:] *= sig_s_el_sab[g]/sig_s_el[g]

  sig_s_el_freegas = transfer_sig_s_el_freegas @ np.ones(G)
  ...
  # Plot the matrix
  if plot:
    Atest = transfer_sig_s_el_freegas
    nz = np.nonzero(Atest)
  ...
  return_data["sigma_s_el_freegas"] = sig_s_el_freegas
...
if __name__ == "__main__":
  ...
  njoy_dir = "njoy_xs"
  chi_dir = "/Users/janv4/Desktop/ChiTech/XSConverters/Njoy2ChiConverter/chi_xs"
    ...
    if njoy_file.endswith(".txt"):
      ...
      if "Cnat" in njoy_path and "296" in njoy_path:
        if any([gs in njoy_path for gs in ["lanl187","lanl618","xmas172"]]):
          ...
          data = BuildCombinedData(raw_njoy_data,True)
          ...
          sigma_s_el_freegas = data["sigma_s_el_freegas"][:len(E)]
          ...
          plt.semilogx(E[::-1], sigma_s_el_freegas, '-xb', ms=2.5, label="$\sigma_{s,el}$-freegas")
          ...
          plt.semilogx(elasto[:,0]*1.0e-6,elasto[:,1])
